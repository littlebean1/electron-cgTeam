<template>
    <div>
        <div style="height: 40px;">
            <div style="float: left; margin: 30px 0 30px 0; font-size:26px">我的设计任务列表({{ designTasksList.length }})</div>
        </div>
        <el-table :data="designTasksList" border tooltip-effect="dark" style="width: 100%;">
...
import { it } from "node:test";
  ...
  export default {
    ...
    mounted() {
        this.getTasksByDesignerId()
    },
    methods: {
        getTasksByDesignerId() {
            let userId = "657fdd5e0d372068c4ec37f2"
            axios.get(
                "http://localhost:3000/api/task/getTasksByDesignerId", {
                    params: {designerId: "657fdd5e0d372068c4ec37f2"},
                }
            ).then((res) => {
                res.data.forEach((item,index)=>{
                    res.data[index].id = index + 1 
                })
                this.designTasksList = res.data
